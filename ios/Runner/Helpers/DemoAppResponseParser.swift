...
class DemoAppResponseParser {
	...
	/// Stringifies response data to pretty printed JSON.
	/// - Parameters:
	///   - data: `Data?` object, data.
	///   - rootJsonKey: `String?` object, root JSON key, default is 'json',
	/// - Returns: `String?` object, pretty printed stringified JSON.
	...
	/// - Parameters:
	///   - data: `Data?` object, data.
	/// - Returns: `[String: Any]? ` object, JSON.
	static func convertToJSON(from data: Data?) -> [String: Any]? {
		if let data = data, let jsonData = try? JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
			return jsonData
			// swiftlint:enable force_try
		}
